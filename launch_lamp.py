...
def launchServer():
    ...
    launch_description = "( alsasrc ! queue leaky=downstream max-size-buffers=16 ! audio/x-raw,format=S16LE,rate=44100,channels=2 ! audioconvert ! vorbisenc quality=0.7 ! queue leaky=downstream max-size-buffers=16 ! rtpvorbispay name=pay0 pt=96 )"

    factory = GstRtspServer.RTSPMediaFactory.new()
    factory.set_launch(launch_description)
    factory.set_shared(True)
    print(factory.get_launch())
    mount_points = server.get_mount_points()
    mount_points.add_factory('/mic', factory)

    server.attach(None)
    print('Stream ready at: ' + str(server.get_address()))
    GLib.MainLoop().run()
